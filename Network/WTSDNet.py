...
from module import DRB,FFM,subnet
...
class WTSDNet(nn.Module):
    ...
    def __init__(self, in_chn=3, wf=64, depth=5, relu_slope=0.2, hin_position_left=0, hin_position_right=4):
        ...
        for i in reversed(range(depth - 1)):
            ...
            prev_channels = (2**i)*wf
        self.ff12 = FF(prev_channels)
        self.cat12 = nn.Conv2d(prev_channels*2, prev_channels, 1, 1, 0)

        #self.last = conv3x3(prev_channels, in_chn, bias=True)
        self.tail = conv3x3(80, in_chn, bias=True)

        #self.sr = RSNet(n_feat=64)
        self.spn = subnet()
        self.ffm = FFM(n_feat=64)
    ...
    def forward(self, x):
        ...
        x1, x1_up = self.down_path_1[0](x1)
        encs.append(x1_up)
        x1, x1_up = self.down_path_1[1](x1)
        encs.append(x1_up)
        x1, x1_up = self.down_path_1[2](x1)
        encs.append(x1_up)
        x1, x1_up = self.down_path_1[3](x1)
        encs.append(x1_up)
        x1= self.down_path_1[4](x1)

        x1 = self.up_path_1[0](x1, self.skip_conv_1[0](encs[- 1]))
        decs.append(x1)
        x1 = self.up_path_1[1](x1, self.skip_conv_1[1](encs[- 2]))
        decs.append(x1)
        x1 = self.up_path_1[2](x1, self.skip_conv_1[2](encs[- 3]))
        decs.append(x1)
        x1 = self.up_path_1[3](x1, self.skip_conv_1[3](encs[- 4]))
        decs.append(x1)
        ...
        xf = self.ffm(encs, decs)
        #print(x2.shape)
        x2 = self.spn(x2)
        x2 = torch.cat((xf,x2),dim=1)
